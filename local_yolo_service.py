import os
import time
from typing import Any, List, Dict, Optional # Import List and Dict for clearer type hints
...
MIN_YOLO_REQUEST_INTERVAL = 0.5 # seconds between YOLO inference calls (can be faster than API)
            ...
            if first_result.boxes is not None:
                for box in first_result.boxes:
                    ...
                    predictions_list.append({
                        ...
                        "width": width,
                        "height": height,
                        "class": first_result.names[int(box.cls[0])], # Get class name from model.names
                        "confidence": float(box.conf[0]),
                        "class_id": int(box.cls[0]),
                    })
            print(f"YOLO: Local inference complete. Got {len(predictions_list)} predictions.")
        else:
            print("YOLO WARNING: No inference results returned from local model.")

    except Exception as e:
        print(f"YOLO ERROR: An unexpected error occurred during local YOLO inference: {e}")
