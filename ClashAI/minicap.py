...
class Minicap:
    ...
    def get_rotation(self) -> int:
        ...
        output = self.adb.shell("dumpsys window | grep -E 'mCurrentRotation|mRotation'").stdout
    ...
    def install(self, local_bin_path: str, local_so_path: str) -> bool:
        ...
        import subprocess
        try:
            subprocess.run(["adb", "-s", self.device_id, "push", local_bin_path, self.bin_path], check=True)
            subprocess.run(["adb", "-s", self.device_id, "push", local_so_path, self.so_path], check=True)
    ...
    def check_installation(self):
        ...
        if bin_exists and so_exists:
            ...
            if test_res.success and test_res.stdout.strip().startswith("{"):
                ...
                print(f"Hint: Ensure you pushed the correct minicap.so for SDK {sdk} and ABI {abi}.")
        else:
            print("Minicap status: NOT INSTALLED")
            print(f"Hint: Push 'minicap' (ABI: {abi}) and 'minicap.so' (SDK: {sdk}, ABI: {abi}) to {self.work_dir}")
        print("---------------------------------------------------------")
    ...
    def take_screenshot(self, filename: str) -> bool:
        ...
        return result.success and os.path.exists(filename) and os.path.getsize(filename) > 0
